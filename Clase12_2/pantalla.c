...
int listarprecionombre(Pantalla* array, int limite)
{
    int flag=1, i;
    Pantalla aux;
    while(flag)
    {
        ...
        for(i=0;i<limite;i++)
        {
            if(!array[i].isEmpty && array != NULL)
            {
                ...
                else if((array[i].precio==array[i+1].precio) && (strcmp(array[i].nombre,array[i+1].nombre)>0))
                {
                        flag=1;
                        aux = array[i];
                        array[i] = array[i+1];
    ...
    for(i=0;i<limite;i++)
    {
        if(!array[i].isEmpty)
        {
            printf("NOMBRE : %s - PRECIO: %f - ID: %d - TIPO: %d\n", array[i].nombre,array[i].precio,array[i].idPantalla,array[i].tipo);
        }
    }
...
/// Listar todas las pantallas que que su valor es inferior o igual a 1000
int listarPrecioMil(Pantalla* array, int limite)
{
    int i;
    for(i=0;i<limite;i++)
    {
...
///Listar todas las pantallas que superan el promedio de los precios

int promedioPrecio(Pantalla* array, int limite)
{
    int i, acumulador=0;
    float acumuladorPrecio=0, promedio=0,retorno=1;
    if(limite > 0 && array != NULL)
    {
        ...
        promedio=acumuladorPrecio/acumulador;
        retorno=promedio;
    }
...
int listarPantallaSupProm(Pantalla* array, int limite)
{
    int i,retorno=1;
    float promedio;
    if(limite>0 && array!=NULL)
    {
        promedio=promedioPrecio(array, limite);
        for(i=0;i<limite;i++)
        {
            if(!array[i].isEmpty)
            {
